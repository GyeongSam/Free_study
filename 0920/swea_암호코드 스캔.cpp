...
int main() {
	...
	while (++tc <= T) {
		...
		for (int i = 0; i < N; ++i) {
			for (l = 0; l < M; ++l) {
				if (board[i][l] != '0') {
					...
					for (int di = 0; board[i + di][l] != '0'; ++di) {
		...
		for (int i = 0; i < secret.size(); ++i) {
			...
			printf("%d ½ÃÅ©¸´Å° %d,%d,%d,%d,%d,%d,%d,%d (%d) Å°°ª:%d ÃÑÇÕ:%d\n", i, secret_key[0], secret_key[1], secret_key[2], secret_key[3], secret_key[4], secret_key[5], secret_key[6], secret_key[7], is_ok(), secret_key[0] + secret_key[1] + secret_key[2] + secret_key[3] + secret_key[4] + secret_key[5] + secret_key[6] + secret_key[7],SUM);
			if (is_ok()) {
